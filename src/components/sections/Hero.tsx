...
export default function Hero({ onNavigate }: HeroProps) {
  ...
  return (
    <section 
      id="home" 
      className="relative overflow-hidden border-b-4 border-black dark:border-white bg-white dark:bg-zinc-950 pt-12 pb-20 px-4 md:px-8 lg:pt-24 lg:pb-32 transition-colors duration-200"
    >
      {/* Decorative neobrutalist floating grids/shapes in background */}
      <div className="absolute top-10 right-10 w-28 h-28 bg-brand-secondary border-4 border-black rounded-full shadow-sm -z-10 animate-bounce" style={{ animationDuration: '4s' }}></div>
      <div className="absolute bottom-10 left-12 w-36 h-20 bg-brand-primary border-4 border-black rotate-12 -z-10 rounded-lg"></div>
      <div className="absolute top-1/2 left-1/3 w-12 h-12 bg-brand-tertiary border-4 border-black rounded-full -z-10 animate-pulse"></div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div className="space-y-6 lg:col-span-7 relative z-10 text-left">
          ...
          <p className="text-lg md:text-xl font-medium text-zinc-800 dark:text-zinc-300 max-w-2xl leading-relaxed">
            A passionate{' '}
            <span className="bg-brand-primary border-2 border-dashed border-black px-1.5 py-0.5 rounded-sm font-bold text-black">
              Business Analyst
            </span>{' '}
            and{' '}
            <span className="bg-brand-tertiary border-2 border-dashed border-black px-1.5 py-0.5 rounded-sm font-bold text-white">
        ...
        <div className="lg:col-span-5 relative w-full max-w-sm mx-auto lg:ml-auto">
          ...
          <div className="absolute -bottom-6 -left-6 bg-brand-tertiary text-black px-4 py-2 border-4 border-black rounded-lg font-bold text-xs shadow-md rotate-[-6deg] hidden sm:block">
            📍 Based in Indonesia
          </div>
          <div className="absolute -top-6 right-2 bg-brand-secondary text-black p-2.5 border-4 border-black rounded-lg shadow-md rotate-[12deg] hidden sm:flex items-center gap-1">
